...
ticketPriceTotal = runners.reduce((acc, curVal)=> {
...
// Problem 1 - filter - Special shoutout to Donations $100+
let donationShoutOut = [];

donationShoutOut = runners.filter((obj)=> {
  return obj.donation >= 100;
});

console.log(donationShoutOut);

// Problem 2 - Map - Donation Matching by 50% then obtain total donations

let donationPlusMatch = [];

donationPlusMatch = runners.map((obj)=> {
  return obj.donation * 1.5;
});

let totalDonation = donationPlusMatch.reduce((total, donation)=> {
  return total + donation;
}, 0);

console.log(totalDonation); 

// Problem 3 - LastName, FirstName

let lastNameFirstName = [];

runners.forEach(function(element) {
  lastNameFirstName.push(`${element.last_name}, ${element.first_name}`);
});
